...
Purpose: This file handles real-time group tracking visualizations
...
class Visualization(object):
    ...
    def draw_trackers(self, tracks):
        ...
        track_ids = [t.group_track_id for t in tracks if t.is_confirmed() and t.time_since_update == 0 and t.group_track_id != 0]
        u = np.unique(track_ids)
        track_ids_updated=[]
        ...
        for i in range(0,len(u)):
            ...
            for j in range(0,len(track_ids)):
                if(track_ids[j] == u[i]):
                    idx.append(track_ids[j])
            if(len(idx) > 1):
                track_ids_updated.append(u[i])
        
        for track in tracks:
            if not track.is_confirmed() or track.time_since_update > 0:
                continue
 
            self.viewer.color = create_unique_color_uchar(track.group_track_id)
