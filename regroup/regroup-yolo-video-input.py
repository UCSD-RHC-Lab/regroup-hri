...
'''
Filename: regroup-video-input.py
Purpose: This file runs REGROUP and YOLO in real-time on video input (no pre-stored pedestrian detections)
...
Run: python regroup-yolo-video-input.py --sequence_dir=../data/test/[sequence] --display=1 --video_path=<path to video>
...
def extractImages(path_to_video, path_to_output_images):
    vidcap = cv2.VideoCapture(args.video_path)
    success,image = vidcap.read()
    count = 1
    
    while success:
      img_name = path_to_output_images + '/rgb/%05d.png'% count
      cv2.imwrite(img_name, image)     # save frame as PNG file      
      success,image = vidcap.read()
      count += 1
...
def create_detections(detection_mat, frame_idx, image,net, layer_names, labels, confidence, threshold, min_height=0):
    ...
    features = encoder(image, boxes.copy())
    detection_list = []
   
    for bbox, feature, conf in zip(boxes, features, confidences):
...
def run(sequence_dir, output_file, min_confidence,
        ...
        nn_budget, display, img_output_path, cif, gp_dist, h_ratio, encoder, cost_metric,
        net, layer_names, labels, confidence):
    """
    ...
    def frame_callback(vis, frame_idx):
        ...
        if frame_idx in seq_info["rgb_image_filenames"]:
            ...
            detections = create_detections(
                seq_info["detections"], frame_idx,image, net, layer_names, labels, 
                confidence, nms_max_overlap, min_height=min_detection_height)
...
def parse_args():
    ...
    parser.add_argument(
        "--output_file", help="Path to the tracking output file.", default="/tmp/hypotheses.txt")
    parser.add_argument(
        "--min_confidence", help="Detection confidence threshold. Disregard ", default=0.8, type=float)
    parser.add_argument(
        "--min_detection_height", help="Threshold on the detection bounding", default=0, type=int)
    parser.add_argument(
        "--cif", help="Crowd indication feature threshold" , default=4, type=int)
    parser.add_argument(
        "--nms_max_overlap",  help="Non-maxima suppression threshold: Maximum", default=0.5, type=float)
    parser.add_argument(
        "--max_cosine_distance", help="Gating threshold for cosine distance", type=float, default=0.2)
    parser.add_argument(
        "--nn_budget", help="Maximum size of the appearance descriptors", type=int, default=100)
    parser.add_argument(
    ...
    parser.add_argument(
        '--weights', type=str, default='./darknet/model/yolov3.weights', help='Path to model weights')
    parser.add_argument(
        '--config', type=str, default='./darknet/model/yolov3.cfg', help='Path to configuration file')
    parser.add_argument(
        '--labels', type=str, default='./darknet/model/coco.names', help='Path to label file')
    parser.add_argument(
        '--confidence', type=float, default=0.5, help='Minimum confidence for a box to be detected.')
    parser.add_argument(
        '--threshold', type=float, default=0.3, help='Threshold for Non-Max Suppression')
    parser.add_argument(
        '--use_gpu', default=False, action='store_true', help='Use GPU (OpenCV must be compiled for GPU)')
    parser.add_argument(
        '--save', default=False, action='store_true', help='Whether or not the output should be saved')
    parser.add_argument(
        '--show', default=True, action="store_false", help='Show output')
    parser.add_argument(
        '--image_path', type=str, default='', help='Path to the image file.')
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    # Read video data and save images
    extractImages(args.video_path, args.sequence_dir)
    ...
    # Run group tracker
    run(
        args.sequence_dir, args.output_file, args.min_confidence, args.nms_max_overlap, 
        args.min_detection_height, args.distance_metric, args.max_cosine_distance, args.nn_budget, 
        args.display, args.img_output_path, args.cif, args.gp_dist, args.h_ratio, encoder, 
        args.cost_metric, net, layer_names, labels, args.confidence)
